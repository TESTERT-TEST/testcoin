...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("GRMS RPC client version") + " " + _(VERUS_VERSION) + "\n" + PrivacyInfo();
        if (!mapArgs.count("-version")) {      strUsage += "\n" + _("Usage:") + "\n" +
                  "  grms-cli [options] <command> [params]  " + _("Send command to Komodo") + "\n" +
                  "  grms-cli [options] help                " + _("List commands") + "\n" +
                  "  grms-cli [options] help <command>      " + _("Get help for a command") + "\n";
